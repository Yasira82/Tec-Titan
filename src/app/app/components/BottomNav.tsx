...
import { Icon, type TitanIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type TitanTab = 'home' | 'modules' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: TitanTab; onSelect: (t: TitanTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: TitanTab; icon: TitanIconName; label: string }[] = [
    { key: 'home',     icon: 'building', label: t.titan.nav.home     },
    { key: 'modules',  icon: 'grid',     label: t.titan.nav.modules  },
    { key: 'pro',      icon: 'star',     label: t.titan.nav.pro      },
    { key: 'settings', icon: 'settings', label: t.titan.nav.settings },
  ];
  return (
    <nav style={{
